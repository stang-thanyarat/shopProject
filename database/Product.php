...
Class Product
{
    ...
    {
        $sql = "INSERT INTO product (product_name, category_id, brand, model, sell_id, img, product_detail, product_img, product_detail_img, product_dlt_unit, product_unit, price, cost_price, notification_amt, sales_status, product_rm_unit, product_exchange_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        $stmt = $this->conn->prepare($sql);
    ...
    public function update($data)
    {
        ...
        $stmt->bindParam(15, $data['product_rm_unit'], PDO::PARAM_INT);
        $stmt->bindParam(16, $data['product_exchange_id'], PDO::PARAM_INT);
        $stmt->bindParam(17, $data['product_id'], PDO::PARAM_STR);
        $stmt->execute();
    }
}
