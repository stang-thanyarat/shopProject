...
class Product
{
    ...
    public function insert($data)
    {
        ...
        $stmt = $this->conn->prepare($sql);
        $stmt->execute();
        $sql = "INSERT INTO product_tb (product_name, category_id, brand, model, sell_id, product_detail, product_img, product_detail_img, product_dlt_unit, product_unit, price, cost_price, notification_amt, sales_status, set_n_amt, date_n_amt, notification_amt2, product_rm_unit/*, product_exchange_id*/) 
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?/*,?*/)";
        $stmt = $this->conn->prepare($sql);
    ...
    public function update($data)
    {
        ...
        SET product_name = ?, category_id = ?, brand = ?, model = ?, sell_id = ?, product_detail = ?, product_dlt_unit = ?, product_unit = ?, price = ?, cost_price = ?, notification_amt = ?, set_n_amt = ?, date_n_amt = ?, notification_amt2 = ?
        WHERE product_id=?";
            $paramCount = 14;
            $stmt = $this->conn->prepare($sql);
            $stmt->bindParam(1, $data['product_name'], PDO::PARAM_STR);
            ...
            $stmt->bindParam(15, $data['product_id'], PDO::PARAM_INT);
            $stmt->execute();
        } catch (Exception $e) {
            http_response_code(500);
            echo strval($e);
