...
class Budget
{
   ...
   */
    function fetchBetweenSales($firstdate=null, $lastdate=null)
    {
    ...
    function fetchBetweenOrder($firstdate=null, $lastdate=null)
    {
        if(is_null($firstdate)){
            $firstdate = date('Y-m-d');
        }
        ...
        $sql = "SELECT O.*,O.payment_sl as payment_sl_order FROM order_tb O WHERE order_id AND O.payment_dt BETWEEN ? AND ? OR O.payment_dt BETWEEN ? AND ?";
        $stmt = $this->conn->prepare($sql);
        $stmt->bindParam(1, $firstdate, PDO::PARAM_STR);
        $stmt->bindParam(2, $lastdate, PDO::PARAM_STR);
        $stmt->bindParam(3, $firstdate, PDO::PARAM_STR);
